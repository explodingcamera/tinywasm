...
impl InterpreterRuntime {
    pub(crate) fn exec(&self, store: &mut Store, stack: &mut Stack) -> Result<()> {
        ...
        loop {
            ...
            match cf.func_instance.instructions.get(cf.instr_ptr).expect("") {
                Nop => { /* do nothing */ }
                ...
                Call(v) => {
                    ...
                    let new_call_frame =
                        CallFrame::new(wasm_func.clone(), func_inst.owner, params, stack.blocks.len() as u32);

                    cf.instr_ptr += 1; // skip the call instruction
                    stack.call_stack.push(core::mem::replace(&mut cf, new_call_frame))?;
                    if cf.module_addr != module.id() {
                        module.swap_with(cf.module_addr, store);
                    }
                    continue;
                ...
                LocalGet3(a, b, c) => {
                    stack.values.extend_from_slice(&[cf.get_local(*a), cf.get_local(*b), cf.get_local(*c)])
                }
                LocalTeeGet(a, b) => {
                    let (a, b) = (*a, *b);
                    let last = stack
