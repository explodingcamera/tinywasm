...
impl Instruction {
    /// Returns the largest module-local memory index used by this instruction.
    #[inline]
    pub fn memory_addr(&self, data: &super::WasmFunctionData) -> Option<MemAddr> {
        use Instruction::*;
        match *self {
            IncMemoryLocal32(arg)
            | IncMemoryLocal64(arg)
            | StoreLocalLocal32(arg)
            | StoreLocalLocal64(arg)
            | StoreLocalLocal128(arg)
            | LoadLocal32(arg)
            | LoadLocal64(arg)
            | LoadLocal8S32(arg)
            | LoadLocal8U32(arg)
            ...
            | LoadLocalTee32(arg)
            | LoadLocalSet32(arg)
            | LoadLocalTee8S32(arg)
            | LoadLocalTee8U32(arg)
            | LoadLocalTee16S32(arg)
            | LoadLocalTee16U32(arg)
            | LoadLocalSet8S32(arg)
            | LoadLocalSet8U32(arg)
            | LoadLocalSet16S32(arg)
            | LoadLocalSet16U32(arg)
            | LoadLocalTee128(arg)
            | LoadLocalSet128(arg) => Some(arg.memory_arg_idx.get(data).mem_addr()),
            I32Load(index)
            | I64Load(index)
            | F32Load(index)
            | F64Load(index)
            ...
            | V128Load32x2S(index)
            | V128Load32x2U(index)
            | V128Load8Splat(index)
            | V128Load16Splat(index)
            | V128Load32Splat(index)
            | V128Load64Splat(index)
            | V128Load32Zero(index)
            | V128Load64Zero(index)
            | V128Store(index) => Some(index.get(data).mem_addr()),
            V128Load8Lane(arg) | V128Load16Lane(arg) | V128Load32Lane(arg) | V128Load64Lane(arg)
            | V128Store8Lane(arg) | V128Store16Lane(arg) | V128Store32Lane(arg) | V128Store64Lane(arg) => {
                Some(arg.memory_arg_idx.get(data).mem_addr())
            }
            FMaStoreF32(arg) | FMaStoreF64(arg) => Some(arg.mem_addr()),
            MemorySize(memory) | MemoryGrow(memory) | MemoryFill(memory) => Some(memory),
            MemoryFillConst(index) => Some(index.get(data).memory),
            MemoryInit(index) => Some(index.get(data).second),
            MemoryCopy(index) => {
                let value = index.get(data);
                Some(value.first.max(value.second))
            }
            _ => None,
        }
    }
