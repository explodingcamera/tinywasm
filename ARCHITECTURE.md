...
Memory growth keeps the Wasm page count and limits on `MemoryInstance`. Before the backing allocation is resized, the configured `ResourceLimiter` is consulted so a host can bound guest memory consumption. The limiter is shared across the stores created from one `Engine` and lives behind an `Arc`.
